...
func New(ctx context.Context, config *Config) (*Handler, error) {
	//fmt.Println("in proxy/vless/outbound func New ")
...
func (h *Handler) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	//fmt.Println("in proxy/vless/outbound func (h *Handler) Process ")
	...
	if err := retry.ExponentialBackoff(5, 200).On(func() error {
		...
		//fmt.Printf("dest: %+v\n%+v\n", rec.Destination().Address, err)
		// 匹配逻辑：
		// dial tcp 后面紧跟的内容
		// ([\d\.]+) 匹配并捕获 IP 地址（数字和点）
		// :[\d]+ 匹配端口号
		// : i/o 确保匹配的是第一个 i/o 之前的那个
		//re := regexp.MustCompile(`dial tcp (\[[^\]]+\]|[\d\.]+):\d+: i/o`)

		//matches := re.FindStringSubmatch(err.Error())

		//if len(matches) > 1 {
		//	fmt.Println("提取的 IP:", matches[1]) // 输出: 108.162.193.16
		//	go h.serverPicker.RemoveServer(matches[1])
		//} else {
		//	fmt.Println("未找到匹配地址")
		//}
